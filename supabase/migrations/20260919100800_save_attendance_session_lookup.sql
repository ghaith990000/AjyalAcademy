-- Ajyal Academy — save_attendance: report an unknown session as not found for admins too.
--
-- `can_view_session` is true for every admin, whether or not the row exists, so the existence check has to
-- come from the row lock itself. Behaviour is otherwise unchanged (see 20260919100700_sessions_attendance.sql).
...
-- create or replace keeps the grants from the original migration; restated so this file stands on its own.
