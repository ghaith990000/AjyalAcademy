...
- **Registration (Phase 10):** the mock seeds five requests (waiting with both CPR warnings, waiting with very long names and no location, waiting with a condition, accepted, rejected), answers `public_locations`, `submit_player_applications` (recording every attempt in `world.submitAttempts`; `failNextSubmit` drops the connection or turns the parent away once), `player_application_overview`, the waiting count and the two decision functions. The audit visits the form signed out (and with four children and every error showing), the five request pages and the accept/reject dialogs; flows cover a parent registering, a dropped connection with a retry, the home card and menu badge, a request arriving live, accepting (with the WhatsApp link), rejecting, and a taken CPR; a CSP test sends the form.
...
- `08_applications.test.sql` runs anonymous calls (`tests.act_as_anon()`), so it also proves what `anon` cannot do; it inserts 60 and 300 rows to test the rate limits and deletes `player_applications` first (everything is rolled back).
